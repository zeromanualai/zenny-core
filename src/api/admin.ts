...
export async function registerAdminRoutes(app: FastifyInstance) {
  ...
  app.get('/admin/analytics/:slug', async (request: FastifyRequest) => {
    ...
    const resolved = convoStats?.filter((c: any) => c.resolved).length || 0;
    const escalated = convoStats?.filter((c: any) => c.escalated).length || 0;
    const totalCost = convoStats?.reduce((sum: number, c: any) => sum + (c.llm_cost_usd || 0), 0) || 0;

    // Model usage breakdown
    const modelUsage: Record<string, number> = {};
    convoStats?.forEach((c: any) => {
